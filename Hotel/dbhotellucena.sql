DROP DATABASE IF EXISTS `HotelLucena`;
...
CREATE TABLE `usuarios` (
  ...
  `tipoUsuariol` enum('Gerente', 'Atendente', 'Funcionário', 'Cliente') DEFAULT NULL,
...
CREATE TABLE `itens` (
  ...
  `codigoReserva` int(11) DEFAULT NULL,
   CONSTRAINT `FK_itens_reservas` FOREIGN KEY (`codigoReserva`) REFERENCES `reservas` (`codigoReserva`)
);

insert into hotellucena.usuarios(codUsuario, contatoUsuario, nomeUsuario, tipoUsuariol, email, senha)
value (1, 'contato', 'gerente', 'Gerente', 'gerente@gmail.com', '12345');
